	...
	const bool debug)
{
  ...
  return std::make_tuple(rendered, num_buckets, out_color, out_invdepth, out_alpha, radii, geomBuffer, binningBuffer, imgBuffer, sampleBuffer);
}
	...
	const bool debug)
{
  ...
  torch::Tensor out_num_pixels_touched = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::countTouchedPixels(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
		...
		tan_fovy,
		prefiltered,
		out_num_pixels_touched.contiguous().data_ptr<int>(),
		antialiasing,
		debug);
  }
  return std::make_tuple(rendered, out_num_pixels_touched);
}
