...
CudaRasterizer::SampleState CudaRasterizer::SampleState::fromChunk(char *& chunk, size_t C) {
	...
	obtain(chunk, sample.ara, C * BLOCK_SIZE, 128);
...
int CudaRasterizer::Rasterizer::countTouchedPixels(
	...
	bool debug)
{
	...
	CHECK_CUDA(FORWARD::countTouchedPixels(
		tile_grid, block,
		imgState.ranges,
	...
	bool debug)
{
	...
	CHECK_CUDA(FORWARD::render(
		...
		sampleState.T, sampleState.ar, sampleState.ara, sampleState.ard,
		width, height,
		(float2*)means2D,
		feature_ptr,
		geomState.conic_opacity,
