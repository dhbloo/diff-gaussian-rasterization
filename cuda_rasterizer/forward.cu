...
renderCUDA(
	...
	float* __restrict__ sampled_ara, float* __restrict__ sampled_ard,
	int W, int H,
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (j % 32 == 0) {
				...
				sampled_ara[(bbm * BLOCK_SIZE) + block.thread_rank()] = accum_alpha;
...
// Find how many pixels in the mask do all the gaussians touched
__global__ void __launch_bounds__(BLOCK_X * BLOCK_Y)
countTouchedPixelsCUDA(
	const uint2* __restrict__ ranges,
	const uint32_t* __restrict__ point_list,
	int W, int H,
	const float2* __restrict__ points_xy_image,
	...
	int* __restrict__ num_pixels_touched)
{
	// Identify current tile and associated min/max pixel range.
	auto block = cg::this_thread_block();
	uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X;
	...
	bool inmask = inside && mask_image[pix_id] > 0.0f;
	// Done threads can help with fetching, but don't rasterize
	bool done = !inmask;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Add 1 to the points mark for this Gaussian
			atomicAdd(&num_pixels_touched[collected_id[j]], 1);
		}
	}
}
...
void FORWARD::render(
	...
	float* sampled_T, float* sampled_ar, float* sampled_ara, float* sampled_ard,
	int W, int H,
	...
	float* depth)
{
	renderCUDA<NUM_CHANNELS_3DGS> << <grid, block >> > (
		...
		sampled_T, sampled_ar, sampled_ara, sampled_ard,
		W, H,
		means2D,
		colors,
		conic_opacity,
...
void FORWARD::countTouchedPixels(
	const dim3 grid, dim3 block,
	const uint2* ranges,
	const uint32_t* point_list,
	int W, int H,
...
{
	countTouchedPixelsCUDA << <grid, block >> > (
		ranges,
		point_list,
		W, H,
		means2D,
